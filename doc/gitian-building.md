...
*Setup instructions for a Gitian build of Magacoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Magacoin
...
- [Building Magacoin Core](#building-magacoin-core)
...
Clone the git repositories for magacoin and Gitian.
...
in the magacoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Magacoin Core
----------------

To build Magacoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the magacoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/magacoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/magacoin/magacoin
...
For example:
```bash
URL=https://github.com/thrasher-/magacoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit magacoin=${COMMIT} --url magacoin=${URL} ../magacoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit magacoin=${COMMIT} --url magacoin=${URL} ../magacoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit magacoin=${COMMIT} --url magacoin=${URL} ../magacoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the magacoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../magacoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/magacoin
SIGPATH=/some/root/path/magacoin-detached-sigs

./bin/gbuild --url magacoin=${BTCPATH},signature=${SIGPATH} ../magacoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
in `gitian.sigs.maga` to your signing machine and do

```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/magacoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/magacoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/magacoin-osx-build.assert
```
...
[gitian.sigs.maga](https://github.com/magacoin/gitian.sigs.maga/) repository, or if that's not possible create a pull
