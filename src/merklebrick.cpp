...
CMerkleBrick::CMerkleBrick(const CBrick& brick, CBloomFilter& filter)
{
    header = brick.GetBrickHeader();

    vector<bool> vMatch;
    vector<uint256> vHashes;

    vMatch.reserve(brick.vtx.size());
    vHashes.reserve(brick.vtx.size());

    for (unsigned int i = 0; i < brick.vtx.size(); i++)
    {
        const uint256& hash = brick.vtx[i].GetHash();
        if (filter.IsRelevantAndUpdate(brick.vtx[i]))
        {
...
CMerkleBrick::CMerkleBrick(const CBrick& brick, const std::set<uint256>& txids)
{
    header = brick.GetBrickHeader();
...
uint256 CPartialMerkleTree::ExtractMatches(std::vector<uint256> &vMatch, std::vector<unsigned int> &vnIndex) {
    ...
    if (nTransactions > MAX_BRICK_BASE_SIZE / 60) // 60 is the lower bound for the size of a serialized CTransaction
        return uint256();
