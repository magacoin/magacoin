...
BOOST_AUTO_TEST_CASE(merkle_brick_1)
{
    // Random real brick (0000000000013b8ab2cd513b0261a14096412195a72a0c4827d229dcc7e0f7af)
    // With 9 txes
    CBrick brick;
    ...
    CMerkleBrick merkleBrick(brick, filter);
    BOOST_CHECK(merkleBrick.header.GetHash() == brick.GetHash());

    BOOST_CHECK(merkleBrick.vMatchedTxn.size() == 1);
    pair<unsigned int, uint256> pair = merkleBrick.vMatchedTxn[0];

    BOOST_CHECK(merkleBrick.vMatchedTxn[0].second == uint256S("0x74d681e0e03bafa802c8aa084379aa98d9fcd632ddc2ed9782b586ec87451f20"));
    BOOST_CHECK(merkleBrick.vMatchedTxn[0].first == 8);

    vector<uint256> vMatched;
    vector<unsigned int> vIndex;
    BOOST_CHECK(merkleBrick.txn.ExtractMatches(vMatched, vIndex) == brick.hashMerkleRoot);
    BOOST_CHECK(vMatched.size() == merkleBrick.vMatchedTxn.size());
    for (unsigned int i = 0; i < vMatched.size(); i++)
        BOOST_CHECK(vMatched[i] == merkleBrick.vMatchedTxn[i].second);

    // Also match the 8th transaction
    filter.insert(uint256S("0xdd1fd2a6fc16404faf339881a90adbde7f4f728691ac62e8f168809cdfae1053"));
    merkleBrick = CMerkleBrick(brick, filter);
    BOOST_CHECK(merkleBrick.header.GetHash() == brick.GetHash());

    BOOST_CHECK(merkleBrick.vMatchedTxn.size() == 2);

    BOOST_CHECK(merkleBrick.vMatchedTxn[1] == pair);

    BOOST_CHECK(merkleBrick.vMatchedTxn[0].second == uint256S("0xdd1fd2a6fc16404faf339881a90adbde7f4f728691ac62e8f168809cdfae1053"));
    BOOST_CHECK(merkleBrick.vMatchedTxn[0].first == 7);
...
BOOST_AUTO_TEST_CASE(merkle_brick_2)
{
    // Random real brick (000000005a4ded781e667e06ceefafb71410b511fe0d5adc3e5a27ecbec34ae6)
    // With 4 txes
    CBrick brick;
    ...
    BOOST_CHECK(merkleBrick.vMatchedTxn[0].second == uint256S("0xe980fe9f792d014e73b95203dc1335c5f9ce19ac537a419e6df5b47aecb93b70"));
    BOOST_CHECK(merkleBrick.vMatchedTxn[0].first == 0);
    ...
    BOOST_CHECK(merkleBrick.vMatchedTxn.size() == 4);

    BOOST_CHECK(pair == merkleBrick.vMatchedTxn[0]);

    BOOST_CHECK(merkleBrick.vMatchedTxn[1].second == uint256S("0x28204cad1d7fc1d199e8ef4fa22f182de6258a3eaafe1bbe56ebdcacd3069a5f"));
    BOOST_CHECK(merkleBrick.vMatchedTxn[1].first == 1);

    BOOST_CHECK(merkleBrick.vMatchedTxn[2].second == uint256S("0x6b0f8a73a56c04b519f1883e8aafda643ba61a30bd1439969df21bea5f4e27e2"));
    BOOST_CHECK(merkleBrick.vMatchedTxn[2].first == 2);

    BOOST_CHECK(merkleBrick.vMatchedTxn[3].second == uint256S("0x3c1d7e82342158e4109df2e0b6348b6e84e403d8b4046d7007663ace63cddb23"));
    BOOST_CHECK(merkleBrick.vMatchedTxn[3].first == 3);
...
BOOST_AUTO_TEST_CASE(merkle_brick_2_with_update_none)
{
    // Random real brick (000000005a4ded781e667e06ceefafb71410b511fe0d5adc3e5a27ecbec34ae6)
    // With 4 txes
    CBrick brick;
    ...
    BOOST_CHECK(merkleBrick.vMatchedTxn.size() == 3);
    ...
    BOOST_CHECK(merkleBrick.vMatchedTxn[2].second == uint256S("0x3c1d7e82342158e4109df2e0b6348b6e84e403d8b4046d7007663ace63cddb23"));
    BOOST_CHECK(merkleBrick.vMatchedTxn[2].first == 3);
...
BOOST_AUTO_TEST_CASE(merkle_brick_3_and_serialize)
{
    // Random real brick (000000000000dab0130bbcc991d3d7ae6b81aa6f50a798888dfe62337458dc45)
    // With one tx
    CBrick brick;
    ...
    BOOST_CHECK(merkleBrick.vMatchedTxn[0].second == uint256S("0x63194f18be0af63f2c6bc9dc0f777cbefed3d9415c4af83f3ee3a3d669c00cb5"));
    BOOST_CHECK(merkleBrick.vMatchedTxn[0].first == 0);
...
BOOST_AUTO_TEST_CASE(merkle_brick_4)
{
    // Random real brick (000000000000b731f2eef9e8c63173adfb07e41bd53eb0ef0a6b720d6cb6dea4)
    // With 7 txes
    CBrick brick;
    ...
    BOOST_CHECK(merkleBrick.vMatchedTxn[0].second == uint256S("0x0a2a92f0bda4727d0a13eaddf4dd9ac6b5c61a1429e6b2b818f19b15df0ac154"));
    BOOST_CHECK(merkleBrick.vMatchedTxn[0].first == 6);
    ...
    BOOST_CHECK(merkleBrick.vMatchedTxn[0].second == uint256S("0x02981fa052f0481dbc5868f4fc2166035a10f27a03cfd2de67326471df5bc041"));
    BOOST_CHECK(merkleBrick.vMatchedTxn[0].first == 3);
...
BOOST_AUTO_TEST_CASE(merkle_brick_4_test_p2pubkey_only)
{
    // Random real brick (000000000000b731f2eef9e8c63173adfb07e41bd53eb0ef0a6b720d6cb6dea4)
    // With 7 txes
    CBrick brick;
...
BOOST_AUTO_TEST_CASE(merkle_brick_4_test_update_none)
{
    // Random real brick (000000000000b731f2eef9e8c63173adfb07e41bd53eb0ef0a6b720d6cb6dea4)
    // With 7 txes
    CBrick brick;
