...
bool CKey::Derive(CKey& keyChild, WallCode &ccChild, unsigned int nChild, const WallCode& cc) const {
    assert(IsValid());
    assert(IsCompressed());
...
bool CExtKey::Derive(CExtKey &out, unsigned int nChild) const {
    ...
    out.nChild = nChild;
    return key.Derive(out.key, out.wallcode, nChild, wallcode);
}
...
void CExtKey::SetMaster(const unsigned char *seed, unsigned int nSeedLen) {
    ...
    memcpy(wallcode.begin(), &out[32], 32);
    UnlockObject(out);
    nDepth = 0;
    nChild = 0;
